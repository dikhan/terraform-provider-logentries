...
func getInsightTagFromData(data *schema.ResourceData) *insight.Tag {
	var patterns []string
	...
	var actions []*insight.Action
	for _, id := range data.Get("action_ids").(*schema.Set).List() {
		actions = append(actions, &insight.Action{Id: id.(string)})
	}
	var sources []*insight.Source
	for _, id := range data.Get("source_ids").(*schema.Set).List() {
		sources = append(sources, &insight.Source{Id: id.(string)})
	}
	var labels []*insight.Label
	for _, id := range data.Get("label_ids").(*schema.Set).List() {
		labels = append(labels, &insight.Label{Id: id.(string)})
	}
	return &insight.Tag{
		Id:       data.Id(),
...
func setInsightTagData(data *schema.ResourceData, tag *insight.Tag) error {
	var labels []string
